...
void nav_callback(const ardrone_autonomy::Navdata& msg_in)
{
	...
	// For debugging
	//ROS_INFO("Sensor readings: vx: %f - vy: %f - vz: %f ");	
}
...
geometry_msgs::Twist test_controller(double vx_des, double vy_des, double vz_des, double K)
{
	...
	twist_msg_gen.linear.x = K * (vx_des - vx); // {-1 to 1} = K*( m/s - m/s)
	//ROS_INFO("vx des - vx sensor &f",vx_des-vx_);
	twist_msg_gen.linear.y  = K * (vy_des - vy); 
	twist_msg_gen.linear.z  = K * (vz_des - vz);
