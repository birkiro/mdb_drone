...
geometry_msgs::Twist tag_controller(float Kx, float Ky, float Kz, float Kangle)
{
	...
	if(tags_count){
		...
		vz_pub = Kz*(1650 - alt);
		vy_pub = Ky*(xc_des - tags_xc[0]);			// tag detection X parameter is Y axis of velocity
		vx_pub = Kx*(yc_des - tags_yc[0]);			// tag detection Y parameter is X axis of velocity
		if (yaw_pub > 0.34)	yaw_pub = 0.34;
		if (yaw_pub < -0.34)	yaw_pub = -0.34;
		...
		printf("xc:%d,yc:%d,zc:%d,ang:%d,vx:%f,vy:%f,vz:%f,angZ:%f\n", tags_xc[0],tags_yc[0],tags_distance[0],tags_orientation[0],vx_pub,vy_pub,vz_pub, yaw_pub);
	}
	else {
		vz_pub = Kz*(1600 - alt);
		if (vz_pub < -0.3) 	vz_pub = -0.3;
...
int main(int argc, char** argv)
{
	...
	while (ros::ok()) 
	{
		...
		{	
			...
			twist_msg = tag_controller(Kx, Ky, Kz, Kangle);
