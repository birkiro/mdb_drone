...
int main(int argc, char** argv)
{
	...
	while (ros::ok()) 
	{
		...
		{	
			if((double)ros::Time::now().toSec() < start_time + takeoff_time + fly_time/4 || 
				(double)ros::Time::now().toSec() > start_time + takeoff_time + fly_time/2 &&
				(double)ros::Time::now().toSec() < start_time + takeoff_time + (fly_time*3)/4)
			{
